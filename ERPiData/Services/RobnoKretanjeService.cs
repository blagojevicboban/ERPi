...
/// <summary>
/// Servis za Robna (Artikal-bazirana) interna kretanja — Primopredaje/Zaduženja/Razduženja
/// (vidi <see cref="RobnoKretanjeNalog"/>). 1:1 po strukturi sa <see cref="PrimopredajaService"/>
/// (Materijalno), ali nad Artikal šifarnikom. Knjiži se preko iste <see cref="MaterijalnaKarticaService"/>
/// jer je <see cref="MaterijalnaKartica"/> tabela knjigovodstveno zajednička za Robno i Materijalno
/// (razlikuje se samo po tome da li je SifraArtikla iz Artikal ili Materijal šifarnika — vidi
/// <see cref="RobniBrutoBilansService"/>), pa nema potrebe za posebnom "robnom karticom" tabelom.
/// </summary>
public class RobnoKretanjeService
{
    private readonly ErpiDbContext _db;

    public RobnoKretanjeService(ErpiDbContext db)
    {
        _db = db;
    }

    public async Task<List<RobnoKretanjeNalog>> GetKretanjaAsync(string vrstaDokumenta, string? search = null)
    {
        var query = _db.RobnaKretanja
            .Include(p => p.Stavke).ThenInclude(s => s.Artikal)
            .Include(p => p.MagacinDaje)
            .Include(p => p.MagacinPrima)
            .Where(p => p.VrstaDokumenta == vrstaDokumenta)
            .AsQueryable();
        ...
        return await query.OrderByDescending(p => p.Datum)
            .ThenByDescending(p => p.RobnoKretanjeNalogId)
            .ToListAsync();
    }

    public async Task<RobnoKretanjeNalog> SaveKretanjeAsync(RobnoKretanjeNalog nalog)
    {
        if (nalog.RobnoKretanjeNalogId == 0)
        {
            if (nalog.BrojNaloga == 0)
            {
                nalog.BrojNaloga = (await _db.RobnaKretanja
                    .Where(p => p.VrstaDokumenta == nalog.VrstaDokumenta)
                    .Select(p => (int?)p.BrojNaloga).MaxAsync() ?? 0) + 1;
            }
            _db.RobnaKretanja.Add(nalog);
        }
        else
        {
            var existing = await _db.RobnaKretanja
                .Include(p => p.Stavke)
                .FirstOrDefaultAsync(p => p.RobnoKretanjeNalogId == nalog.RobnoKretanjeNalogId);

            if (existing != null)
            {
                if (existing.IsKnjizen)
                    throw new InvalidOperationException($"Proknjižen(a) {existing.VrstaDokumenta.ToLower()} se ne može menjati.");
                ...
                _db.RobnaKretanjaStavke.RemoveRange(existing.Stavke);
    ...
    public async Task ObrisiKretanjeAsync(int robnoKretanjeNalogId)
    {
        var nalog = await _db.RobnaKretanja
            .Include(p => p.Stavke)
            .FirstOrDefaultAsync(p => p.RobnoKretanjeNalogId == robnoKretanjeNalogId);

        if (nalog == null) return;
        if (nalog.IsKnjizen)
            throw new InvalidOperationException($"Proknjižen(a) {nalog.VrstaDokumenta.ToLower()} se ne može obrisati — prvo rasknjižite.");

        _db.RobnaKretanjaStavke.RemoveRange(nalog.Stavke);
        _db.RobnaKretanja.Remove(nalog);
        await _db.SaveChangesAsync();
    }

    public async Task KnjiziKretanjeAsync(int robnoKretanjeNalogId)
    {
        var nalog = await _db.RobnaKretanja
            .Include(p => p.Stavke).ThenInclude(s => s.Artikal)
            .Include(p => p.MagacinDaje)
        ...
        if (nalog == null) throw new InvalidOperationException("Dokument nije pronađen.");
        if (nalog.IsKnjizen) throw new InvalidOperationException($"{nalog.VrstaDokumenta} je već proknjižen(a).");
        if (nalog.MagacinDaje == null || nalog.MagacinPrima == null)
            throw new InvalidOperationException("Magacini moraju biti izabrani.");
        ...
        foreach (var s in nalog.Stavke)
        {
            ...
            decimal vrednost = await kartice.DodajIzlazRedAsync(
                ...
                sifraArtikla,
                nalog.Datum,
                $"{nalog.VrstaDokumenta} br. {nalog.BrojNaloga} u magacin {sifraPrima}",
                s.Kolicina);
            ...
            await kartice.DodajUlazRedAsync(
                ...
                sifraArtikla,
                nalog.Datum,
                $"{nalog.VrstaDokumenta} br. {nalog.BrojNaloga} iz magacina {sifraDaje}",
                s.Kolicina,
                jedinicaCena);
    ...
    private async Task<int> KreirajNalogPrelazaVpMpAsync(RobnoKretanjeNalog nalog, Magacin magDaje, Magacin magPrima, decimal vrednostDaje, decimal vrednostPrima)
    {
    ...
    public async Task RasknjiziKretanjeAsync(int robnoKretanjeNalogId)
    {
        var nalog = await _db.RobnaKretanja
            .Include(p => p.Stavke).ThenInclude(s => s.Artikal)
            .Include(p => p.MagacinDaje)
        ...
        if (nalog == null) throw new InvalidOperationException("Dokument nije pronađen.");
        if (!nalog.IsKnjizen) throw new InvalidOperationException($"{nalog.VrstaDokumenta} nije proknjižen(a).");
        if (nalog.MagacinDaje == null || nalog.MagacinPrima == null)
            throw new InvalidOperationException("Magacini nisu pronađeni.");
