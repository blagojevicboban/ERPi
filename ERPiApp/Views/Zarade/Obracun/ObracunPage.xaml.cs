...
public partial class ObracunPage : Page
{
    ...
    private void GeneratePdfReport(ObracunPlate o, string filePath)
    {
        Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                try
                {
                    using var dbDetails = ERPiData.ErpiDbContext.Create(ERPiApp.AppConfig.DbPath);
                ...
                try
                {
                    using var db = ERPiData.ErpiDbContext.Create(ERPiApp.AppConfig.DbPath);
                    var firma = db.Firme.FirstOrDefault();
                    if (firma != null)
                    {
                        nazivFirme = (firma.Naziv + " " + firma.PttIMesto).Trim().ToUpper();
                        if (string.IsNullOrWhiteSpace(nazivFirme)) nazivFirme = "NAZIV FIRME";
                        podaciFirme = $"(PIB: {firma.Pib ?? "-"}, MB: {firma.MaticniBroj ?? "-"})";
                    }
                }
                catch {}
    ...
    private void PrimeniStorno(bool storniraj)
    {
        ...
        string? razlog = ERPiApp.Services.Zarade.RazlogPrompt.Trazi(Window.GetWindow(this),
        ...
        {
            using var db = ERPiData.ErpiDbContext.Create(ERPiApp.AppConfig.DbPath);
            var servis = new ERPiApp.Services.Zarade.StornoService(db);
            ...
            var isplata = o.IsplataId.HasValue
                ...
                : new ERPiApp.Services.Zarade.IsplataService(db).Isplate(o.Godina, o.Mesec, RodIsplate.Zarada).FirstOrDefault();
    ...
    private void BtnIzveziXml_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (sfd.ShowDialog() == true)
        {
            try
            {
                var xmlService = new ERPiApp.Services.Zarade.XmlExportService();
                ...
                try
                {
                    ...
                    if (firma != null)
                    {
                        ...
                        adresa = $"{firma.Adresa} {firma.PttIMesto}".Trim();
                        email = firma.Email ?? "";
                    }
                }
                catch {}
