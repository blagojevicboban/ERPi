...
public class ObracunViewModel : INotifyPropertyChanged
{
    ...
    public async Task ZakljucajSveAsync()
    {
        try
        {
            ...
            var provera = new ERPiApp.Services.Zarade.PreFlightService(_db).Proveri(SelectedGodina, SelectedMesec);
            if (!ERPiApp.Services.Zarade.PreFlightPrompt.DozvoliZakljucavanje(provera))
            {
                StatusText = $"Zaključavanje zaustavljeno — {ERPiApp.Services.Zarade.PreFlightPrompt.OpisZaAudit(provera)}.";
                return;
            }
            ...
            if (res == System.Windows.MessageBoxResult.Yes)
            {
                ...
                ERPiApp.Services.Zarade.AuditService.Zabelezi(_db, SelectedGodina, SelectedMesec, AkcijaObracuna.Zakljucan,
                    $"{Obracuni.Count} obračuna, {ERPiApp.Services.Zarade.PreFlightPrompt.OpisZaAudit(provera)}");
