...
public partial class ListiciPage : Page
{
    ...
    private async void BtnPosaljiEmail_Click(object sender, RoutedEventArgs e)
    {
        ...
        bool zastiti = false; // UserSettings nije dostupan u ERPiApp — podrazumevano isključeno
        ...
        try
        {
            using var db = ERPiData.ErpiDbContext.Create(AppConfig.DbPath);
            using var posiljalac = new ERPiApp.Services.Zarade.SmtpPosiljalac();
            ...
            var servis = new ERPiApp.Services.Zarade.ListicEmailService(db, posiljalac);
    ...
    private static ERPiApp.Services.Zarade.SmtpPodesavanja UcitajSmtpPodesavanja()
    {
        // UserSettings nije dostupan u ERPiApp — SMTP se konfiguriše ručno
        return new ERPiApp.Services.Zarade.SmtpPodesavanja
        {
            Server = "",
            Port = 587,
            KoristiSsl = true,
    ...
    private static string SastaviIzvestaj(ERPiApp.Services.Zarade.IzvestajSlanja izvestaj)
    {
        ...
        var problematicni = izvestaj.Stavke
            .Where(s => s.Ishod != ERPiData.Models.Zarade.IshodSlanja.Poslato)
            .Take(15)
            .ToList();
