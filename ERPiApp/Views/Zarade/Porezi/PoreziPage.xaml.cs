...
namespace ERPiApp.Views.Zarade.Porezi;

public partial class PoreziPage : Page
{
    private ErpiDbContext _db;
    private ERPiData.Models.Zarade.Porezi? _currentParams;
    ...
    private void UcitajParametre(int godina, int mesec)
    {
        try
        {
            ...
            if (_currentParams == null)
            {
                ...
                if (fallback != null)
                {
                    _currentParams = new ERPiData.Models.Zarade.Porezi
                    {
                        Godina = godina,
                        Mesec = mesec,
                        RedniBroj = 1,
        ...
        try
        {
            ...
            if (radniSatiList.Count > 0)
            {
                ...
                if (rez == MessageBoxResult.Yes)
                {
                    try
                    {
                        var obracunService = new ERPiApp.Services.Zarade.ObracunService(_db);
