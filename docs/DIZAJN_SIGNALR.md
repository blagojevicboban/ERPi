...
| Promena statusa porudžbine (Poslata/Isporučena...) | ✅ **dodato u Fazi 2 (11.K)** | Ista cev kao "nova porudžbina" — event `statusPorudzbine`, emituje `AdminController.PromeniStatusPorudzbine` / `KreirajPosiljku` pored postojećeg email/SMS-a; Web Admin osveži listu isto kao na "novaPorudzbina". Druga otvorena admin sesija vidi promenu bez ručnog "Osveži". |
